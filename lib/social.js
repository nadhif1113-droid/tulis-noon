...
/**
 * Leaderboard Tantangan Launch 15 Hari — top user berdasarkan challengeXp.
 * Hanya tampilin user yg punya challengeXp > 0 (udah ikutan tantangan).
 * Returns: [{ id, displayName, avatarEmoji, challengeXp, xp }]
 */
export async function getChallengeLeaderboard(n = 10) {
  try {
    // Buffer 3x lebih besar karena leaderboard hanya tampilkan user yang
    // sudah REGISTER event (isi data diri + setuju anti-cheat).
    const q = query(
      collection(firestore, 'users'),
      where('challengeXp', '>', 0),
      orderBy('challengeXp', 'desc'),
      limit(n * 3)
    );
    const snap = await getDocs(q);
    const all = snap.docs.map((d) => {
      const data = d.data();
      const reg = data.eventRegistration;
      return {
        id: d.id,
        displayName: reg?.fullName || data.displayName || 'Anonim',
        avatarEmoji: data.avatarEmoji || null,
        challengeXp: data.challengeXp || 0,
        xp: data.xp || 0,
        eventScore: data.eventStats?.totalScore || 0,
        isRegistered: !!reg?.agreedAt,
        registeredEventId: reg?.eventId || null,
        city: reg?.city || null,
      };
    });
    // Hanya user yang sudah daftar event ini yang masuk leaderboard
    const eligible = all.filter((u) => u.isRegistered);
    return eligible.slice(0, n);
  } catch (e) {
    console.warn('getChallengeLeaderboard error:', e);
    return [];
  }
}

/**
 * Peringkat user di Tantangan Launch (1-based). Pakai count aggregate.
 */
export async function getUserChallengeRank(myChallengeXp) {
  if (!myChallengeXp || myChallengeXp <= 0) return null;
  try {
    const q = query(collection(firestore, 'users'), where('challengeXp', '>', myChallengeXp));
    const snap = await getCountFromServer(q);
    return (snap.data().count || 0) + 1;
  } catch (e) {
    console.warn('getUserChallengeRank error:', e);
    return null;
  }
}
 ...
 */
/**
 * Posting baru ke komunitas.
 * @param {object} user — { uid, name, avatarEmoji }
 * @param {string} text — isi posting (max 500 char)
 * @param {string} visibility — 'public' (default) atau 'friends' (cuma teman bisa lihat)
 */
export async function createPost(user, text, visibility = 'public') {
 ...
 *   - 'friends' → cuma kalau currentUserId === author OR currentUserId di friendIds penulis
 *
 * @param {number} n — limit (default 30)
 * @param {object} opts — { currentUserId, friendIds: string[] }
 */
export async function getPosts(n = 30, opts = {}) {
  const { currentUserId, friendIds = [] } = opts;
  const friendSet = new Set(friendIds);
  try {
    const q = query(collection(firestore, 'posts'), orderBy('createdAt', 'desc'), limit(n * 2));
    const snap = await getDocs(q);
    const all = snap.docs.map((d) => {
      const p = d.data();
      return {
        ...
        createdAtMs: p.createdAt?.toDate ? p.createdAt.toDate().getTime() : 0,
      };
    });
    // Filter friends-only posts
    const visible = all.filter((p) => {
      if (p.visibility !== 'friends') return true;
...
/**
 * Hitung jumlah post baru sejak lastCheckMs (untuk badge unread di tab Sosial).
 * Filter visibility sama kayak getPosts.
 *
 * @param {number} lastCheckMs — timestamp last visit komunitas
 * @param {object} opts — { currentUserId, friendIds }
 */
export async function getUnreadPostCount(lastCheckMs, opts = {}) {
  if (!lastCheckMs) return 0;
  const { currentUserId, friendIds = [] } = opts;
  const friendSet = new Set(friendIds);
  try {
    const q = query(collection(firestore, 'posts'), orderBy('createdAt', 'desc'), limit(50));
    const snap = await getDocs(q);
    let count = 0;
    for (const d of snap.docs) {
      const p = d.data();
      const t = p.createdAt?.toDate ? p.createdAt.toDate().getTime() : 0;
      if (t <= lastCheckMs) continue;
      // Skip own posts (gak perlu notif diri sendiri)
      if (p.authorId === currentUserId) continue;
      // Filter visibility
      const vis = p.visibility || 'public';
      if (vis === 'friends' && !friendSet.has(p.authorId)) continue;
      count++;
    }
    return count;
  } catch (e) {
    console.warn('getUnreadPostCount error:', e);
    return 0;
  }
}
