...
/**
 * Send test notification 5 seconds from now — buat verify sistem jalan
 * tanpa nunggu waktu sholat beneran.
 */
export async function sendTestNotification() {
  ...
  try {
    const { Capacitor } = await import('@capacitor/core');
    if (!Capacitor.isNativePlatform()) {
      console.log('Test notif cuma jalan di native app');
      return false;
    }
    const { LocalNotifications } = await import('@capacitor/local-notifications');
    ...
    if (permStatus.display !== 'granted') {
      const newPerm = await LocalNotifications.requestPermissions();
      if (newPerm.display !== 'granted') return false;
    }

    const testTime = new Date(Date.now() + 5000); // 5 detik dari sekarang
    const hadis = getRandomHadis();

    await LocalNotifications.schedule({
      notifications: [{
        id: 999999,
        title: '🕌 Test Notifikasi Tulis Noon',
        body: hadis ? `"${hadis.text}" — ${hadis.source}` : 'Notifikasi sholat sistem berfungsi!',
        schedule: { at: testTime },
        sound: 'default',
        extra: { type: 'test' },
      }],
    });
    console.log('Test notif scheduled for', testTime.toLocaleTimeString());
    return true;
  } catch (e) {
