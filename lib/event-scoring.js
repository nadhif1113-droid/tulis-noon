...
// 🏆 GRAND PRIZE — Juara 1 punya threshold extra: harus capai 4000 XP.
// Kalau gak ada user yang capai ini, hadiah juara 1 di-rollover ke juara 2 (admin discretion).
...
export function checkGrandPrizeEligibility(profile, eventStartMs, now = Date.now()) {
  ...
  if (!baseCheck.eligible) {
    return {
      eligible: false,
      reason: 'Belum lulus syarat dasar (lihat eligibility utama)',
      currentScore, requiredScore: GRAND_PRIZE_MIN_SCORE, gap, percent,
    };
  }
  ...
  return {
    ...
    currentScore, requiredScore: GRAND_PRIZE_MIN_SCORE, gap: 0, percent: 100,
  };
}
