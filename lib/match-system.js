...
/**
 * 4 LEVEL Match Arena — berdasarkan XP pemain.
 * Tiap level: rentang XP, pool bot, level soal (qLevel), & reward.
 * Makin tinggi level → lawan lebih pinter, soal lebih sulit, reward lebih besar.
 */
export const MATCH_LEVELS = [
  { idx: 0, id: 'permulaan', label: 'Permulaan', tierName: 'Mubtadi',     emoji: '🌱', minXp: 0,     maxXp: 10000,    botLevels: [1, 2, 3],  qLevel: 1, reward: { win: 25,  lose: 8,  tie: 15, coin: 1 }, color: '#0a4d3c', bgGradient: 'linear-gradient(135deg, #0a4d3c, #1a6b56)', desc: 'Kenal huruf hijaiyah' },
  { idx: 1, id: 'mudah',     label: 'Mudah',     tierName: 'Daris',       emoji: '📖', minXp: 10001, maxXp: 25000,    botLevels: [3, 4, 5],  qLevel: 2, reward: { win: 45,  lose: 12, tie: 25, coin: 2 }, color: '#1a6b56', bgGradient: 'linear-gradient(135deg, #1a6b56, #2e8869)', desc: 'Kosakata dasar & angka' },
  { idx: 2, id: 'menengah',  label: 'Menengah',  tierName: 'Mutawassith', emoji: '🎯', minXp: 25001, maxXp: 50000,    botLevels: [5, 6, 7],  qLevel: 3, reward: { win: 70,  lose: 18, tie: 35, coin: 3 }, color: '#c9a961', bgGradient: 'linear-gradient(135deg, #c9a961, #d4b876)', desc: 'Pasar & sehari-hari' },
  { idx: 3, id: 'mahir',     label: 'Mahir',     tierName: 'Faaheem',     emoji: '🏆', minXp: 50001, maxXp: Infinity, botLevels: [8, 9, 10], qLevel: 4, reward: { win: 100, lose: 25, tie: 50, coin: 5 }, color: '#a05536', bgGradient: 'linear-gradient(135deg, #a05536, #c46a3f)', desc: 'Frasa & istilah islami' },
];

/**
 * Tentukan level match dari XP pemain.
 */
export function getMatchLevelForXp(xp) {
  const x = Math.max(0, xp || 0);
  return MATCH_LEVELS.find((l) => x >= l.minXp && x <= l.maxXp) || MATCH_LEVELS[0];
}

/**
 * Pick bot sesuai level pemain + win-streak (matchmaking adaptif).
 * Aturan: menang 2x berturut-turut → lawan naik 1 level (lebih sulit).
 * Returns: { bot, level, bumped }
 *   - level: MATCH_LEVELS object yang dipakai (sudah memperhitungkan bump)
 *   - bumped: true kalau level dinaikkan karena win-streak
 */
export function pickBotForMatchLevel(xp = 0, winStreak = 0) {
  const base = getMatchLevelForXp(xp);
  let effIdx = base.idx;
  if (winStreak >= 2) effIdx = Math.min(MATCH_LEVELS.length - 1, base.idx + 1);
  const level = MATCH_LEVELS[effIdx];
  const pool = BOTS.filter((b) => level.botLevels.includes(b.level));
  const bot = pool[Math.floor(Math.random() * pool.length)] || BOTS[0];
  return { bot: { ...bot, matchLevel: level }, level, bumped: effIdx > base.idx };
}
...
export function determineMatchOutcome(userScore, botScore, opponent) {
  ...
  // Pakai reward per-level kalau ada (sistem 4 level baru), fallback ke tier lama.
  const rewards = opponent.matchLevel?.reward || getRewardByTier(opponent.tier);
