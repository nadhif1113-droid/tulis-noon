...
export function AuthProvider({ children }) {
  ...
  async function createUserProfile(firebaseUser, additionalData = {}) {
    try {
      ...
      const profileData = {
        ...
        tourCompleted: false,
        // Mata uang dalam game — bisa di-earn (daily, streak, achievement) atau top-up.
        // Dipakai buat BELI NYAWA EXTRA kalau habis sebelum 24 jam.
        coins: 5, // welcome gift 5 koin untuk user baru
        // Lives/nyawa system — model Duolingo-style.
        // User punya max 10 nyawa per hari. Tiap "kalah" di game = -1.
        // Habis sebelum 24 jam? Beli pakai koin (1 koin = 1 nyawa) atau tunggu reset.
        lives: 10,
        maxLives: 10,
        livesResetAt: null, // Timestamp ISO — kapan terakhir reset (untuk auto-refill check)
        // Match Arena stats — buat smart matchmaking
        matchesPlayed: 0,
        matchesWon: 0,
        // === GLOBAL UNLOCK MODE (launch phase) ===
        // Sementara semua fitur premium TERBUKA untuk semua user baru.
        // Nanti kalau mau monetize ulang, balik ke `false` + reset existing users via migration.
        hafalanFullUnlocked: true,
        perkenalanBundleUnlocked: true,
        unlockedPerkenalanMateri: [],
        completedPerkenalanMateri: [],
        // === Tracking completion untuk ceklis materi (lib/learning-progress.js) ===
        completedLessonModules: {},   // { umrah: [moduleId, ...], profesi: [...], pelajar: [...] }
        completedNahwuShorf: {},      // { nahwu: [lessonId, ...], shorf: [...] }
        completedCeritaStories: [],   // [storyId, ...]
        // hafalanProgress already tracks per-chunk (existing field di bawah)
        // Tanya Cepat quota — free 5 percakapan + bundle 10 per 5 koin
        tanyaCepatFreeUsed: 0,
        tanyaCepatBundleQuota: 0,
        // Learning paths — modul & percakapan yg dibuka user
        unlockedUmrahModules: [],         // ['arah-lokasi', 'miqat-ihram', ...]
        unlockedProfesiModules: [],
        unlockedBeasiswaModules: [],
        unlockedConversations: {},        // { 'arah-lokasi': [5, 6, 7], ... }
        // Streak tracking (real, auto-update via lastStreakDate compare)
        lastStreakDate: null, // format YYYY-MM-DD
        // Daily Tournament — XP earned hari ini
        dailyXp: { date: null, xp: 0 },
        // Tantangan Launch 15 Hari — XP yg dikumpulin selama periode tantangan.
        // Di-increment hanya kalau Date.now() ada di periode (lihat lib/challenge-launch.js).
        challengeXp: 0,
        // === PREMIUM MODEL "Tulis Noon Mahir" ===
        // User baru otomatis dapat trial 10 hari (semua kebuka).
        // Setelah trial habis → balik ke free tier generous.
        // Bayar via lynk.id → admin manual aktifkan premiumExpiresAt / premiumLifetime.
        trialExpiresAt: Date.now() + 10 * 24 * 60 * 60 * 1000,
        premiumExpiresAt: null,
        premiumLifetime: false,
        premiumPlanId: null, // 'coba-3bulan' | 'mahir-1tahun' | 'pendiri-lifetime'
  ...
  async function signInWithGoogle() {
    try {
      // Native (iOS/Android): pakai plugin @capacitor-firebase/authentication.
      // Ini munculin native Google account picker (BUKAN browser eksternal),
      // lalu kita sign-in ke Firebase JS SDK pakai credential-nya.
      let isNative = false;
      try {
      ...
      if (isNative) {
        ...
        if (!idToken && !accessToken) {
          return { success: false, error: 'Google sign-in dibatalkan' };
        }
      ...
      // Web: signInWithPopup (browser popup OAuth)
