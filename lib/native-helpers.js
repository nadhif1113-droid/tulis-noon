...
/**
 * Setup Push Notifications native (iOS + Android).
 * Returns: FCM token (string) atau null kalau gagal/declined.
 *
 * Usage:
 *   const token = await setupPushNotifications();
 *   if (token) await saveTokenToFirestore(userId, token);
 */
export async function setupPushNotifications() {
  const native = await isNative();
  if (!native) return null; // di web, pakai FCM web helper terpisah

  const platform = await getPlatform();

  // ⚠️ Android: SKIP push-notifications sampai google-services.json (Firebase) di-setup.
  // Tanpa Firebase config, PushNotifications.register() akan crash dgn:
  // "Default FirebaseApp is not initialized" → FATAL EXCEPTION: CapacitorPlugins → app crash.
  // Local notifications (waktu sholat) tetep jalan, karena pakai plugin terpisah.
  //
  // Setelah google-services.json ditambah ke android/app/ DAN di-rebuild,
  // set env NEXT_PUBLIC_ENABLE_ANDROID_PUSH=true di Vercel untuk aktifkan push Android.
  if (platform === 'android' && process.env.NEXT_PUBLIC_ENABLE_ANDROID_PUSH !== 'true') {
    console.log('Push notifications disabled on Android — set NEXT_PUBLIC_ENABLE_ANDROID_PUSH=true setelah FCM siap');
    return null;
  }
  ...
  try {
    ...
    // 1. Request permission (iOS only flow di sini)
    ...
    // 2. iOS: Register dengan APNs (gak butuh google-services.json)
    await PushNotifications.register();

    // 3. Listen for token (one-time)
    return new Promise((resolve) => {
      const removeRegHandler = PushNotifications.addListener('registration', (token) => {
        console.log('Push token:', token.value);
        removeRegHandler.then((h) => h?.remove?.());
        resolve(token.value);
      });
      const removeErrHandler = PushNotifications.addListener('registrationError', (err) => {
        console.error('Push registration error:', err);
        removeErrHandler.then((h) => h?.remove?.());
        resolve(null);
      });
...
/**
 * Listen for incoming push notifications saat app foreground.
 * onNotificationReceived({ title, body, data })
 */
export async function addPushNotificationListener(onNotificationReceived) {
  try {
    const { PushNotifications } = await import('@capacitor/push-notifications');
